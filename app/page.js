...
const [oilProducts, setOilProducts] = useState([]);
  const [oilForm, setOilForm] = useState({ name: "", price: "", stock: "" });
  const [editingOilProductId, setEditingOilProductId] = useState(null);
const [oilEditForm, setOilEditForm] = useState({ name: "", price: "", stock: "" });
...
async function deletePersonnel(id) {
  ...
  stock: numberValue(oilForm.stock),
};
...
function cancelEditOilProduct() {
  ...
  setOilEditForm({ name: "", price: "", stock: "" });
}
...
async function saveEditOilProduct(id) {
  ...
  await updateOilProduct(id, "stock", oilEditForm.stock);
...
{automationResult && (
  ...
  {automationResult && (
    <div className="mt-5 grid md:grid-cols-2 gap-4">
      ...
      {active === "yag" && <section className="rounded-3xl bg-slate-900 border border-slate-800 p-5 max-w-4xl"><h3 className="font-black text-xl mb-4">Yağ Cari / Ürün Fiyatları</h3><div className="grid md:grid-cols-[1fr_180px_120px] gap-3 items-end mb-6"><Input label="Ürün Adı" value={oilForm.name} onChange={(v) => setOilForm({ ...oilForm, name: v })} placeholder="Örn: 10W40 Motor Yağı" /><Input label="Satış Fiyatı" value={oilForm.price} onChange={(v) => setOilForm({ ...oilForm, price: v })} placeholder="0" /><Input
  label="Stok Adedi"
  value={oilForm.stock}
  onChange={(v) => setOilForm({ ...oilForm, stock: v })}
  placeholder="0"
/><button onClick={addOilProduct} className="rounded-2xl bg-blue-700 hover:bg-blue-600 px-4 py-3 font-bold flex justify-center gap-2"><Plus className="w-5 h-5" /> Ekle</button></div><div className="space-y-3">{oilProducts.length === 0 && <Empty text="Henüz yağ ürünü eklenmedi." />}{oilProducts.map((p) => (
  <div
    key={p.id}
    className="rounded-2xl bg-slate-950 border border-slate-800 p-4"
  >
    {editingOilProductId === p.id ? (
      <div className="grid md:grid-cols-[1fr_160px_160px_90px_90px] gap-3 items-end">
        <Input
          label="Ürün Adı"
        ...
        <Input
  label="Stok Adedi"
  value={oilEditForm.stock}
  onChange={(v) => setOilEditForm({ ...oilEditForm, stock: v })}
/>
  ...
  Stok: <span className="font-bold text-white">{Number(p.stock || 0)}</span> adet
</div>
        </div>
